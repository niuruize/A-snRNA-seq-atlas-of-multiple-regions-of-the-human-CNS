...
#加载数据
load("~/aging/PFC/merge/EGAD00001008287.RData")
load("~/aging/PFC/merge/GSE140231_PFC.RData")
load("~/aging/PFC/merge/GSE141552_PFC.RData")
load("~/aging/PFC/merge/GSE144136_PFC.RData")
load("~/aging/PFC/merge/GSE157827_PFC.RData")
load("~/aging/PFC/merge/GSE168408_PFC.RData")
load("~/aging/PFC/merge/GSE174367_PFC.RData")
load("~/aging/PFC/merge/GSE213982_PFC.RData")
load("~/aging/PFC/merge/PMID34582785_PFC.RData")
load("~/aging/PFC/merge/PMID36824961_PFC.RData")
load("~/aging/PFC/merge/PRJNA434002_PFC.RData")
load("~/aging/PFC/merge/PRJNA544731_PFC.RData")
load("~/aging/PFC/merge/syn18485175_PFC.RData")
load("~/aging/PFC/merge/syn21125841_PFC.RData")
load("~/aging/PFC/merge/syn38120890_PFC.RData")

#数据整理
EGAD00001008287 = subset(EGAD00001008287, DoubletFinder == "Singlet")
GSE140231_PFC = subset(GSE140231_PFC, DoubletFinder == "Singlet")
GSE141552_PFC = subset(GSE141552_PFC, Diagnosis == "Control" & DoubletFinder == "Singlet")
GSE144136_PFC = subset(GSE144136_PFC, DoubletFinder == "Singlet")
GSE144136_PFC$datasets = "GSE144136_PFC"
GSE157827_PFC = subset(GSE157827_PFC, Diagnosis == "Control" & DoubletFinder == "Singlet")
GSE168408_PFC = subset(GSE168408_PFC, DoubletFinder == "Singlet")
GSE174367_PFC = subset(GSE174367_PFC, Diagnosis == "Control" & DoubletFinder == "Singlet")
GSE213982_PFC = subset(GSE213982_PFC, DoubletFinder == "Singlet")
PMID34582785_PFC = subset(PMID34582785_PFC, DoubletFinder == "Singlet")
PMID36824961_PFC = subset(PMID36824961_PFC, DoubletFinder == "Singlet")
PRJNA434002_PFC = subset(PRJNA434002_PFC, DoubletFinder == "Singlet")
PRJNA544731_PFC = subset(PRJNA544731_PFC, DoubletFinder == "Singlet")
syn18485175_PFC = subset(syn18485175_PFC, Diagnosis == "Control" & DoubletFinder == "Singlet")
syn21125841_PFC = subset(syn21125841_PFC, Diagnosis == "Control" & DoubletFinder == "Singlet")
syn38120890_PFC = subset(syn38120890_PFC, Diagnosis == "Control" & DoubletFinder == "Singlet")


#合并数据
scRNA = merge(EGAD00001008287,y=c(GSE140231_PFC,GSE141552_PFC,GSE144136_PFC,GSE157827_PFC,GSE168408_PFC,GSE174367_PFC,GSE213982_PFC, PMID34582785_PFC,PMID36824961_PFC,PRJNA434002_PFC,PRJNA544731_PFC,syn18485175_PFC,syn21125841_PFC,syn38120890_PFC))
###增加年龄分组信息
Age_Group <-  data.frame(readxl::read_xlsx("age_group.xlsx"))
current.cluster.ids <- Age_Group$SampleID
new.cluster.ids <- Age_Group$Age_Group
scRNA@meta.data$Age_Group <- plyr::mapvalues(x = as.character(scRNA@meta.data$SampleID), from = current.cluster.ids, to = new.cluster.ids)
table(scRNA$Age,scRNA$Age_Group)
...
save(scRNA,file="PFC_merge.RData")
Idents(scRNA)="SampleID"
scRNA = subset(scRNA, idents = "ROS20", invert = TRUE)
rm(EGAD00001008287,GSE140231_PFC,GSE141552_PFC,GSE144136_PFC,GSE157827_PFC,GSE168408_PFC,GSE174367_PFC,GSE213982_PFC,PMID34582785_PFC,PMID36824961_PFC,PRJNA434002_PFC,PRJNA544731_PFC,syn18485175_PFC,syn21125841_PFC,syn38120890_PFC)
...
#选择---PFC---锚点 F-19-DSC8;M-20-DSC16;F-20-GSM5138547;M-21-SRR9262956;F-90-syn21125841_C8;F-80-D19_4115;M-40-Br3942_Ant;M-50-Br5161_DLPFC
scRNA.anchors <- FindIntegrationAnchors(object.list = scRNAlist, reference = c(1,4,54,83,90,108,150,155), anchor.features = 3000,reduction = "rpca", dims = 1:50)

# this command creates an 'integrated' data assay
scRNA <- IntegrateData(anchorset = scRNA.anchors, dims = 1:50)
scRNA
dim(scRNA[["RNA"]]@counts)
dim(scRNA[["RNA"]]@data)
dim(scRNA[["integrated"]]@counts) #因为是从RNA这个assay的data矩阵开始整合的，所以这个矩阵为空
dim(scRNA[["integrated"]]@data)
...
head(Idents(scRNA), 5)
save(scRNA,file="scRNA_PFC_seurat.RData")

#去掉双胞重新降维
scRNA = scRNA[,scRNA$seurat_clusters %in% c("0","1","2","3","4","5","6",       "8","9","10",
...
prop.table(table(scRNA$copykat.pred, scRNA$Group), margin = 2)
prop.table(table(scRNA$seurat_clusters, scRNA$Age_Group), margin = 2)*100
prop.table(table(scRNA$celltype, scRNA$Age_Group), margin = 2)*100


my36colors <- c('#765c91','#da8240','#c4a09f','#9cc28d','#765c91','#bcaf93','#f6f09b','#c39e64',"11111")
p1 <- DimPlot(scRNA, reduction = "umap",group.by = "celltype", cols = my36colors, pt.size=0.01,label = T)+theme(
...
ggsave(filename = "PFC_celltype1.pdf", plot = p1, device = 'pdf', width = 13, height = 11, units = 'cm')
...
{#tsne
  ...
  p1 <- clusterCornerAxes(scRNA,reduction = 'tsne',clusterCol = "seurat_clusters", nrow = 2,groupFacet = 'Age_Group', noSplit = F,pSize=0.01,cellLabel = F, cellLabelSize = 3,show.legend = T, aspect.ratio = 1,themebg = 'bwCorner')
  ggsave(filename = "tsne_Age_Group_1.pdf", plot = p1, device = 'pdf', width = 35, height = 18, units = 'cm')
  ...
  p1 <- clusterCornerAxes(scRNA,reduction = 'tsne',clusterCol = "seurat_clusters", groupFacet = 'Group', noSplit = F,pSize=0.01,cellLabel = F, cellLabelSize = 3,show.legend = T, aspect.ratio = 1,themebg = 'bwCorner')
  ...
  ggsave(filename = "tsne_cluster_1.png", plot = p1, device = 'png', width = 30, height = 25, units = 'cm')
  ...
  p1 <- clusterCornerAxes(scRNA,reduction = 'umap',clusterCol = "seurat_clusters", groupFacet = 'Group', noSplit = F,pSize=0.01,cellLabel = F, cellLabelSize = 3,show.legend = T, aspect.ratio = 1,themebg = 'bwCorner')
  ggsave(filename = "umap_SampleID_3.pdf", plot = p1, device = 'pdf', width = 30, height = 9, units = 'cm')
  p1 <- clusterCornerAxes(scRNA,reduction = 'umap',clusterCol = "seurat_clusters", nrow = 2,groupFacet = 'Age_Group', noSplit = F,pSize=0.01,cellLabel = F, cellLabelSize = 3,show.legend = T, aspect.ratio = 1,themebg = 'bwCorner')
  ggsave(filename = "umap_Age_Group_1.pdf", plot = p1, device = 'pdf', width = 35, height = 18, units = 'cm')
...
{#tsne
  ...
  p1 <- clusterCornerAxes(scRNA,reduction = 'tsne',clusterCol = "celltype", groupFacet = 'Age_Group', noSplit = F, nrow = 2,pSize=0.01,cellLabel = T, cellLabelSize = 3,show.legend = F, aspect.ratio = 1,themebg = 'bwCorner')
  ggsave(filename = "tsne_celltype_Age_group_1.pdf", plot = p1, device = 'pdf', width = 40, height = 20, units = 'cm')
  
  p1 <- DimPlot(scRNA, reduction = "tsne", group.by = "celltype", pt.size=0.01, label = T,repel = TRUE)+theme(
    axis.line = element_blank(),axis.ticks = element_blank(),axis.text = element_blank(),plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
  ggsave(filename = "tsne_celltype_1.pdf", plot = p1, device = 'pdf', width = 25, height = 23, units = 'cm')
  ...
  p1 <- clusterCornerAxes(scRNA,reduction = 'umap',clusterCol = "celltype", groupFacet = 'Age_Group', noSplit = F, nrow = 2,pSize=0.01,cellLabel = T, cellLabelSize = 3,show.legend = F, aspect.ratio = 1,themebg = 'bwCorner')
  ggsave(filename = "umap_celltype_Age_Group_1.pdf", plot = p1, device = 'pdf', width = 40, height = 20, units = 'cm')
...
#单个基因差异基因小提琴图
p1<-VlnPlot(scRNA, features = c("CNTN2"),idents = 'Micro',pt.size = 1,split.by = "Age_Group",ncol = 1)+theme(axis.title.x=element_text(size=0))
ggsave(filename = "PFC_Micro_CNTN2.pdf", plot = p1, device = 'pdf', width = 15, height = 10)
...
scRNA$celltype <- factor(scRNA$celltype, levels=c("ExN","InN","Astro","Astro_Oligo", "Micro","Micro_Astro","Micro_Oligo",
...
##
###为分群重新指定细胞类型 
current.cluster.ids <- c("50", "60", "71", "72", "74", "75", 
                         "77", "80", "87", "91")
new.cluster.ids <- c("Stage40_60","Stage40_60","Stage60_80","Stage60_80","Stage60_80","Stage60_80",
                     "Stage60_80","Stage60_80","Stage80+","Stage80+") 
scRNA@meta.data$Age_stage <- plyr::mapvalues(x = as.integer(as.character(scRNA@meta.data$Age)), from = current.cluster.ids, to = new.cluster.ids)
table(scRNA@meta.data$Age_stage,scRNA@meta.data$Age)
table(scRNA@meta.data$Age_stage,scRNA@meta.data$Sex)
table(scRNA$orig.ident, scRNA$seurat_clusters)
table(scRNA@meta.data$SampleID,scRNA@meta.data$Age)
...
ggsave(filename = "Heatmap10.pdf", plot = Heatmap, device = 'pdf', width = 50, height = 60, units = 'cm')
...
ggsave(filename = "Heatmap5_1.pdf", plot = Heatmap, device = 'pdf', width = 50, height = 40, units = 'cm')
Heatmap <- DoHeatmap(scRNA,features = top5$gene,disp.min=-2.5, disp.max=2.5, group.colors = c("#00BFC4","#AB82FF","#00CD00","#C77CFF"))+
  scale_fill_gradientn(colors = c("white","grey","firebrick3")) 
ggsave(filename = "Heatmap5_2.pdf", plot = Heatmap, device = 'pdf', width = 55, height = 40, units = 'cm')
...
markers.to.plot <- c("PROX1","RBFOX1","SNAP25","SATB2","CAMK2A","SLC17A7","GRIN1","GAD1","GAD2","DCX","SLC1A2","GFAP","AQP4","SOX2","VCAN","SOX6","BCAS1","PLP1","MBP",
                     "CSF1R","P2RY12","CD74","CX3CR1","PTPRC","FLT1","CLDN5","DCN","COL1A2","RELN","CD247","CD4","WDFY4","LSP1","LYZ","F13A1","COLEC12","DNAH9","CFAP54","CLIC6")
...
dev.off()
pdf("cluster_marker_3.pdf", width = 13,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_4.pdf", width = 13,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

DefaultAssay(scRNA) <- "RNA"
markers.to.plot <- c("RBFOX1","SNAP25","SLC17A7","CAMK2A","GAD1","GAD2","RELN",
                     "GFAP","AQP4","CSF1R","CD74","OLIG1","OLIG2","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","ACTA2","TAGLN")
...
markers.to.plot <- c("RBFOX1","SNAP25","SLC17A7","CAMK2A","CUX2","RORB","FOXP2","PCP4","HS3ST4","THEMIS","TLE4",
                     ...
                     "GFAP","AQP4","CSF1R","CD74","VCAN","SOX6","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","ACTA2","TAGLN")
...
DotPlot(scRNA, features = c("CNTN2"), idents = "Micro",cols = c("blue", "red","green"), group.by = "Age_Group", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
...
dev.off()
#L4
markers.to.plot <- c("SNAP25","RORB","SATB2","FOXP2","FOXP1","DCX","NRGN")
pdf("L4_marker_1.pdf", width = 5,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#L5_6_THEMIS
markers.to.plot <- c("SNAP25","PCP4","THEMIS","SATB2","FOXP1","CNR1","NTNG2","SULF1","RGS12","TLL1","ADRA1A")
pdf("L5_6_marker_1.pdf", width = 5,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#L5_6_TLE4
markers.to.plot <- c("SNAP25","TLE4","SOX5","FOXP2","HS3ST4","TUBA1A","NFIB","BCL11B","DCX")
pdf("L5_6_marker_2.pdf", width = 5,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#InN
...
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("CD74","CSF1R"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "Micro_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("MBP","PLP1"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "MOL_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("OLIG1","OLIG2"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "OPC_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("GAD1","GAD2"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "InN_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("SYT1","SLC17A7"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "ExN_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("RELN","SNAP25"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "CR_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("GFAP","AQP4"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "Astro_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("FLT1","CLDN5"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "Endo_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
celltype_umap<- FeaturePlot(scRNA, features = c("DCN","COL1A2"),raster=FALSE,ncol = 1,pt.size = 0.01,cols =c("lightgrey", "#CD0000")) 
ggsave(filename = "Peri_umap_1.png", plot = celltype_umap, device = 'png', width = 9.5, height = 16, units = 'cm')
rm(celltype_umap)
...
p <- Nebulosa::plot_density(scRNA, features = c("CD74","CSF1R"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "Microglia.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("OLIG1","OLIG2"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "OPC.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("MBP","PLP1","BCAS1"),joint = T,reduction = "umap",size = 0.01)[[4]]
ggsave(filename = "NFOL.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("MBP","PLP1"),joint = T,reduction = "umap",size = 0.01)[[3]]
...
p <- Nebulosa::plot_density(scRNA, features = c("SNAP25","SLC17A7"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "ExN.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("GAD1","GAD2"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "InN.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("SNAP25","RELN"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "CR.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("DCN","COL1A2"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "Peri.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
p <- Nebulosa::plot_density(scRNA, features = c("FLT1","CLDN5"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "End.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')

p <- Nebulosa::plot_density(scRNA, features = c("CNTN2","LAPTM5"),joint = T,reduction = "umap",size = 0.01)[[3]]
ggsave(filename = "CNTN2_LAPTM5.pdf", plot = p, device = 'pdf', width = 11, height = 8, units = 'cm')
...
scRNA = scRNA[,scRNA$celltype %in% c("Astro","CR","Endo","ExN","InN","Micro","MOL","OPC","Peri")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA$celltype <- factor(scRNA$celltype,levels=c("ExN","InN","CR","Astro","Micro","OPC","MOL","Endo","Peri"))
...
save(scRNA,file="scRNA_PFC1_seurat.RData")
