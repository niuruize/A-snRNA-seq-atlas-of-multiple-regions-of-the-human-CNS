...
#load data
load("~/Downloads/scRNA/AD_aging/GSE140231_MB/GSE140231_MB.RData")
load("~/Downloads/scRNA/AD_aging/GSE157783_MB/GSE157783_Midbrain.RData")

#合并数据
scRNA = merge(GSE140231_MB, y=c(GSE157783_Midbrain))
scRNA$Region = "MB"
scRNA = subset(scRNA, DoubletFinder == "Singlet")
rm(GSE140231_MB,GSE157783_Midbrain)
scRNA <- subset(scRNA, percent.rb < 10 & percent.HB < 0.25)

#cellinfo <- subset(scRNA@meta.data, select= c("orig.ident","percent.mt","celltype","age","datasets"))
...
##将每个样本的高变基因都调整到上一步选出的2000个
scRNAlist <- PrepSCTIntegration(scRNAlist, anchor.features = scRNA.features)
###数据合并
scRNA.anchors <- FindIntegrationAnchors(object.list = scRNAlist, normalization.method = "SCT",reduction = "rpca", anchor.features = scRNA.features, dims = 1:50)  ##耗时久
save(scRNA.anchors,file="/Users/niuruize/Downloads/scRNA/AD_aging/aging/EC/scRNA.anchors.RData")
scRNA <- IntegrateData(scRNA.anchors, normalization.method = "SCT", dims = 1:50)  ##耗时久
...
scRNA_SCT <- IntegrateData(anchorset = scRNA.anchors, normalization.method = "SCT")
...
save(scRNA,file="scRNA_MB_seurat.RData")
...
scRNA <- JackStraw(scRNA, dims = 50, num.replicate = 100)  ##耗时久
...
scRNA@meta.data[["celltype"]]<-factor(scRNA@meta.data[["celltype"]], levels=c("ExN","InN","DaN","Astro","Micro","OPC","MOL","Endo","Peri","Epend"))

###为分群重新指定细胞类型 
cluster_celltype <-  data.frame(readxl::read_xlsx("cluster_celltype.xlsx"))
current.cluster.ids <- cluster_celltype$cluster
new.cluster.ids <- cluster_celltype$celltype2
...
DefaultAssay(scRNA) <- "integrated"
markers <- FindAllMarkers(scRNA, logfc.threshold = 0.1, min.pct = 0.1, only.pos = FALSE, test.use = "wilcox")  ##耗时久
...
markers.to.plot <- c("RBFOX1","SNAP25","CAMK2A","SLC17A6","SLC17A7","SLC17A8","GRIN1","MEIS2","ALDH1A1","SLC6A3","TH","GRIK1","GAD1","GAD2","DCX","RELN", "GFAP","AQP4","CSF1R","P2RY12","CD74","CX3CR1","PTPRC",
                     "CD247","CD8A","CD3D","CD4","VCAN","SOX6","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","FOXJ1","DNAH9","CFAP54","CLIC6")

pdf("cluster_marker_1.pdf", width = 14,height = 9)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_2.pdf", width = 14,height = 9)
...
dev.off()
pdf("cluster_marker_3.pdf", width = 14,height = 9)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_4.pdf", width = 14,height = 9)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

markers.to.plot <- c("RBFOX1","SNAP25","CAMK2A","SLC17A6","SLC17A7","SLC6A3","TH","GAD1","GAD2","GFAP","AQP4",
                     "CSF1R","CD74","VCAN","SOX6","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","DNAH9","CFAP54","PTPRC","CD247")
pdf("celltype_marker_1.pdf", width = 9.4,height = 4)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "celltype", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("celltype_marker_2.pdf", width = 9.4,height = 4)
...
markers.to.plot <- c("RBFOX1","SNAP25","SATB2","CAMK2A","CUX2","RORB","FOXP2","BCL11B","ADRA1A","HS3ST4","THEMIS","TLE4","RELN",
                     "GAD1","GAD2","NR2F1","LHX6","LAMP5","KIT","NOS1","PVALB","MEPE","PLEKHH2","SST","VIP","CNR1",
                     "GFAP","AQP4","CSF1R","P2RY12","CD74","CX3CR1","PTPRC",
                     "VCAN","SOX6","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2")
...
#ImN
markers.to.plot <- c("SNAP25","MEIS2","UNC5D","FOXG1","EIF1B","MEF2C","DCX")
pdf("ImN_marker_1.pdf", width = 5,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#L2_3
markers.to.plot <- c("SNAP25","CUX2","RASGRF2","CAMK2A","DCX")
pdf("L2_3_marker_1.pdf", width = 5,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#L4
markers.to.plot <- c("SNAP25","RORB","SATB2","FOXP2","FOXP1","DCX")
pdf("L4_marker_1.pdf", width = 5,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#L5_6_THEMIS
markers.to.plot <- c("SNAP25","PCP4","THEMIS","SATB2","FOXP1","CNR1","NTNG2","SULF1","RGS12","TLL1","ADRA1A")
pdf("L5_6_marker_1.pdf", width = 5,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#L5_6_TLE4
markers.to.plot <- c("SNAP25","TLE4","SOX5","FOXP2","HS3ST4","TUBA1A","NFIB","BCL11B","DCX")
pdf("L5_6_marker_2.pdf", width = 5,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
#InN
markers.to.plot <- c("LHX6","NR2F2","SOX6","DLX1","DLX2","GAD1","DCX","PVALB","MEPE","PLEKHH2","PLCL1","NPY","SLIT2","SST","ERBB4","RELN","SV2C","KIT","CNR1","LAMP5","CCK","NOS1","CA1","VIP","NOX4","SCTR","ABI3BP","SCML4","CHRNA2","PENK","CALB2")
pdf("InN_marker_1.pdf", width = 10,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

markers.to.plot <- c("SALL1","P2RY12","MEF2C","TOMM40","CD74","TLR4","NFKB1","C1QB","C1QC","C1QA","P2RY6","TYROBP","IL2RA","IL18","IL4R","IL6ST","TGFBR1","ILRUN","ITGAM","NRROS","SLC7A7")
DotPlot(scRNA, features = markers.to.plot,idents = "Micro",cols = c("blue", "red","green"), group.by = "Group",dot.scale = 8) + RotatedAxis()
...
##保存数据
scRNA = scRNA[,scRNA$celltype %in% c("ExN","InN","Tcell","Astro","Micro","OPC","DaN","MOL","Endo","Peri","Epend")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA@meta.data[["celltype"]]<-factor(scRNA@meta.data[["celltype"]], levels=c("ExN","DaN","InN","Astro","Micro","OPC","MOL","Endo","Peri","Epend","Tcell"))

save(scRNA,file="/Users/niuruize/Downloads/scRNA/AD_aging/aging/Midbrain/Seurat/scRNA_MB_seurat.RData")
