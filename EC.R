...
#数据整理
GSE138852_EC = subset(GSE138852_EC, Diagnosis == "Control")
GSE147528_EC = subset(GSE147528_EC, Tangle.Stage == "Stage 0")
GSE186538_EC = GSE186538

#合并数据
scRNA = merge(GSE138852_EC, y=c(GSE147528_EC,GSE186538_EC))
scRNA = subset(scRNA, DoubletFinder == "Singlet")
rm(GSE138852_EC,GSE147528_EC,GSE186538,GSE186538_EC)
...
##
###为分群重新指定细胞类型 
current.cluster.ids <- c("ExN","InN","CR","Astro","Micro","OPC","NFOL","MOL","Endo","Peri","T")
new.cluster.ids <- c("ExN","InN","CR","Astro","Micro","OPC","NFOL","MOL","Endo","Peri","Tcell")
...
markers.to.plot <- c("MOBP","PLP1","MBP","OPALIN","CNTNAP2","SYT1","RELN",
...
markers.to.plot <- c("RBFOX1","SNAP25","SLC17A7","SATB2","CAMK2A","GAD1","GAD2","RELN","GFAP","AQP4","VCAN","SOX6","BCAS1","PLP1","MBP",
                     "CSF1R","P2RY12","CD74","CX3CR1","DNAH9","CFAP54","FLT1","CLDN5","PDGFRB","DCN","COL1A2","ITGA10","PTPRC","CD247")
...
dev.off()
pdf("cluster_marker_3.pdf", width = 10,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_4.pdf", width = 10,height = 12)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

markers.to.plot <- c("RBFOX1","SNAP25","SLC17A7","CAMK2A","GAD1","GAD2","RELN",
                     "GFAP","AQP4","CSF1R","CD74","VCAN","SOX6","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","PTPRC","CD247")
...
scRNA = scRNA[,scRNA$celltype %in% c("ExN","InN","CR","Astro","Micro","OPC","NFOL","MOL","Endo","Peri")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA$celltype <- factor(scRNA$celltype,levels=c("ExN","InN","CR","Astro","Micro","OPC","NFOL","MOL","Endo","Peri","Tcell"))
save(scRNA,file="/Users/niuruize/Downloads/scRNA/AD_aging/aging/EC/seurat/EC_seurat_filter.RData")
