...
#load data
load("~/aging/astrocyte/ACC_Astro.RData")
load("~/aging/astrocyte/AMY_Astro.RData")
load("~/aging/astrocyte/EC_Astro.RData")
load("~/aging/astrocyte/HIP_Astro.RData")
load("~/aging/astrocyte/MB_Astro.RData")
load("~/aging/astrocyte/PFC_Astro.RData")
load("~/aging/astrocyte/Retina_Astro.RData")
load("~/aging/astrocyte/SC_Astro.RData")

#合并数据
scRNA = merge(ACC_Astro, y=c(AMY_Astro,EC_Astro,HIP_Astro,MB_Astro,PFC_Astro,Retina_Astro,SC_Astro))
rm(ACC_Astro,AMY_Astro,EC_Astro,HIP_Astro,MB_Astro,PFC_Astro,Retina_Astro,SC_Astro)
save(scRNA,file="Astro_merge.RData")
...
scRNA = scRNA[,scRNA$celltype %in% c("Astro1","Astro2","Astro3","Astro4","Astro5","Astro6","Astro7","Astro8","Astro9","Astro10")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA$celltype <- factor(scRNA$celltype,levels = c("Astro1","Astro2","Astro3","Astro4","Astro5","Astro6","Astro7","Astro8","Astro9","Astro10"))
table(scRNA$celltype)
Idents(scRNA) <- 'celltype'
save(scRNA,file="Astro_anno.RData")
...
prop.table(table(scRNA$celltype, scRNA$Region_Group), margin = 2)*100
...
##
###---------------------------------------PFC_Astro--------------------------------#####
pdf("Astro_celltype_Marker1.pdf", width = 3.5,height = 1.6)
markers.to.plot <- c("MFGE8","SOX2","GFAP","AQP4","ETNPPL","EGFR")
DotPlot(scRNA, features = markers.to.plot,group.by = "celltype") + 
  theme_bw()+
  theme(panel.grid = element_blank(), axis.text.x=element_text(hjust = 1,vjust=1,angle=35))+
  labs(x=NULL,y=NULL)+guides(size=guide_legend(order=3))+
  scale_color_gradientn(values = seq(0,1,0.2),colours = c("#3F6699","#FFFFFF","#923331"))
dev.off()

pdf("Astro_celltype_Marker1.pdf", width = 6.5,height = 2.8)
markers.to.plot <- c("SLC1A2","SLC1A3","ETNPPL","SOX2","DPP10","GPC6","EGFR","PDE4B","GFAP","AQP4","CHI3L1","F3","BCL6","CD44","FN1","HSPA9","FOS","JUN")
DotPlot(scRNA, features = markers.to.plot,group.by = "celltype") + 
  theme_bw()+
...
p1 <- DimPlot(scRNA, reduction = "umap", group.by = "Group", pt.size=0.1, label = F,repel = TRUE,cols = my36colors)+theme(
  axis.line = element_blank(),axis.ticks = element_blank(),axis.text = element_blank(),plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
ggsave(filename = "umap_group_1.pdf", plot = p1, device = 'pdf', width = 18, height = 15, units = 'cm')
p1 <- DimPlot(scRNA, reduction = "umap", group.by = "Region", pt.size=0.1, label = F,repel = TRUE,cols = my36colors)+theme(
  axis.line = element_blank(),axis.ticks = element_blank(),axis.text = element_blank(),plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
ggsave(filename = "umap_Region_1.pdf", plot = p1, device = 'pdf', width = 18, height = 15, units = 'cm')
