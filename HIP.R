...
#读取数据
load("GSE163737_HIP.RData")
load("GSE199243_HIP.RData")
load("PMID34582785_HIP.RData")

#数据整理
GSE163737_HIP = subset(GSE163737_HIP, DoubletFinder == "Singlet")
GSE199243_HIP = subset(GSE199243_HIP, DoubletFinder == "Singlet")
PMID34582785_HIP = subset(PMID34582785_HIP, DoubletFinder == "Singlet")

#合并数据
scRNA = merge(GSE163737_HIP, y=c(GSE199243_HIP,PMID34582785_HIP))
save(scRNA,file="merge.RData")
rm(GSE163737_HIP,GSE199243_HIP,PMID34582785_HIP)
scRNA <- subset(scRNA, percent.rb < 10 & percent.HB < 0.25)
p1 = VlnPlot(scRNA, group.by = "SampleID", pt.size = 0,features = c("nFeature_RNA", "nCount_RNA", "percent.mt","percent.rb","percent.HB"), ncol = 1)
ggsave("HIP_1.pdf", p1, width = 40, height = 40, units = "cm")
...
ggsave("HIP_2.pdf", width = 30, height = 25, units = "cm")
rm(plotscRNA_PFC_1, plotscRNA_PFC_2)
...
#选择锚点 M-Adult-Br5161_DLPFC;M-Aging-GSM5292838;F-Adult-GSM4157077;F-Aging-GSM5292839
scRNA.anchors <- FindIntegrationAnchors(object.list = scRNAlist, reference = c(1, 18, 51, 52), anchor.features = 3000,reduction = "rpca", dims = 1:50)
}
{
scRNAlist <- SplitObject(scRNA, split.by = "SampleID")
scRNAlist <- lapply(X = scRNAlist, FUN = function(x) {
...
scRNA
save(scRNA,file="HIP_seurat.RData")
...
markers.to.plot <- c("PROX1","RBFOX1","SNAP25","SATB2","CAMK2A","SLC17A7","GRIN1","GAD1","GAD2","DCX","GFAP","AQP4","OLIG1","OLIG2","BCAS1","PLP1","MBP",
                     "CSF1R","P2RY12","CD74","CX3CR1","PTPRC","CD247","FLT1","CLDN5","DCN","COL1A2","RELN","CLIC6","DNAH9","CFAP54")
...
dev.off()
pdf("cluster_marker_3.pdf", width = 12,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_4.pdf", width = 12,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

markers.to.plot <- c("RBFOX1","SNAP25","SATB2","CAMK2A","GAD1","GAD2","RELN",
                     "GFAP","AQP4","CSF1R","P2RY12","CD74","CX3CR1","PTPRC",
                     "VCAN","SOX6","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2")
pdf("celltype_marker_1.pdf", width = 9,height = 4)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "celltype", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("celltype_marker_2.pdf", width = 9,height = 4)
...
markers.to.plot <- c("SLC1A3","GFAP","SPARC","AQP4","HES5","SOX2","PAX6","HMGB2","ST18","PLP1","MBP","MOBP",
                     "EMX2","FOXG1","EGFR","LPAR1","ETNPPL","MFGE8","SOX5","SOX6","CDK1","ASCL1","TFAP2C",
                     "EOMES","IGFBPL1","CALB2","PLK5","NES","NEUROG2","MCM2","GAD1","GAD2","PROX1","DCX","NEUROD1","CALB1","TOP2A",
                     "ELAVL2","VIM","NOTCH2","SOX4","SOX11",
                     "PADI2","FRZB","WNT8B","MAP2","ADAMTS19","CDK6","WIF1","DNAH9","CFAP54")

pdf("NSC_marker_1.pdf", width = 18,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()
...
scRNA = scRNA[,scRNA$celltype %in% c("ExN","InN","CR","Astro","Micro","OPC","NFOL","MOL","Epend","Endo","Peri","Tcell")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA$celltype <- factor(scRNA$celltype,levels=c("ExN","InN","CR","Astro","Micro","OPC","NFOL","MOL","Epend","Endo","Peri","Tcell"))
save(scRNA,file="/Users/niuruize/Downloads/scRNA/AD_aging/aging/Hip/HIP_seurat_filter.RData")
