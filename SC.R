...
#读取数据
counts <-read.csv('GSE190442_aggregated_counts_postqc.csv.gz', header = TRUE,row.names=1)
metadata <-read.csv('GSE190442_aggregated_metadata_postqc.csv.gz', header = TRUE,row.names=1)
scRNA <- CreateSeuratObject(counts = counts, metadata=metadata, project = "SC", min.cells = 3, min.features = 200) 
#整理基本信息
scRNA$raw_celltype <- metadata$top_level_annotation
scRNA$raw_subtype <- metadata$subtype_annotation
scRNA$SampleID <- metadata$sample
current.sample.ids <- c("1","2","3","4","5","6","7")
new.sample.ids <- c("GSM5723843","GSM5723844","GSM5723845","GSM5723846","GSM5723847","GSM5723848","GSM5723849")
new.sex.ids <- c("M","M","M","M","F","F","F") 
new.age.ids <- c("50","60","65","70","75","80","80") 
new.group.ids <- c("Adult","Adult","Aging","Aging","Aging","Aging","Aging")
scRNA$Sex <- plyr::mapvalues(x = as.integer(as.character(scRNA$SampleID)), from = current.sample.ids, to = new.sex.ids)
scRNA$Age <- plyr::mapvalues(x = as.integer(as.character(scRNA$SampleID)), from = current.sample.ids, to = new.age.ids)
scRNA$SampleID <- plyr::mapvalues(x = as.integer(as.character(scRNA$SampleID)), from = current.sample.ids, to = new.sample.ids)
scRNA$Group <- plyr::mapvalues(x = as.character(scRNA$SampleID), from = new.sample.ids, to = new.group.ids)
table(scRNA$SampleID,scRNA$Age)
table(scRNA$SampleID,scRNA$Sex)
table(scRNA$Group,scRNA$Age)
scRNA <- RenameCells(scRNA, add.cell.id = scRNA$SampleID)
scRNA[["datasets"]] <- "GSE190442_SC"
scRNA[["Diagnosis"]] <- "Control"
scRNA[["Tangle.Stage"]] <- "NA"
scRNA[["APOE"]] <- "NA"
scRNA[["Region"]] <- "SC"
scRNA[["PMI"]] <- "NA"
...
summary(scRNA[[]]$percent.rb)
HB.genes <- c("HBA1","HBA2","HBB","HBD","HBE1","HBG1","HBG2","HBM","HBQ1","HBZ")
HB.genes <- CaseMatch(HB.genes, rownames(scRNA))
scRNA[["percent.HB"]] <- PercentageFeatureSet(scRNA,features = HB.genes)
summary(scRNA[[]]$percent.HB)
rm(current.sample.ids,new.age.ids,new.sample.ids,new.sex.ids,counts,metadata,new.group.ids,HB.genes)
...
ggsave("SC_1.pdf", p1, width = 40, height = 40, units = "cm")
...
ggsave("SC_2_.pdf", width = 30, height = 25, units = "cm")
rm(plotscRNA_PFC_1, plotscRNA_PFC_2)
save(scRNA,file="SC_scRNA.RData")
...
save(scRNA,file="SC_seurat.RData")
...
scRNA <- JackStraw(scRNA, num.replicate = 100)  ##耗时久
scRNA <- ScoreJackStraw(scRNA, dims = 1:20) 
p6<-JackStrawPlot(scRNA, dims = 1:20)
ggsave(filename = "pJackStrawPlot.pdf", plot = p6, device = 'pdf', width = 14, height = 12, units = 'cm')
...
{#tsne
  ...
  p1 <- DimPlot(scRNA, reduction = "tsne", group.by = "celltype", pt.size=0.01, label = T,repel = TRUE)+theme(
    axis.line = element_blank(),axis.ticks = element_blank(),axis.text = element_blank(),plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
  ggsave(filename = "tsne_celltype_1.pdf", plot = p1, device = 'pdf', width = 27, height = 25, units = 'cm')
  p1 <- DimPlot(scRNA, reduction = "tsne", group.by = "celltype", pt.size=0.01, label = F,repel = TRUE)+
    theme(plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
  ggsave(filename = "tsne_celltype_2.pdf", plot = p1, device = 'pdf', width = 27, height = 25, units = 'cm')
  ...
  p1 <- DimPlot(scRNA, reduction = "umap", group.by = "celltype", pt.size=0.01, label = TRUE,repel = TRUE)+theme(
    axis.line = element_blank(),axis.ticks = element_blank(),axis.text = element_blank(),plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
  ggsave(filename = "umap_celltype_1.pdf", plot = p1, device = 'pdf', width = 27, height = 25, units = 'cm')
  p1 <- DimPlot(scRNA, reduction = "umap", group.by = "celltype", pt.size=0.01, label = F,repel = TRUE)+
    theme(plot.title=element_text(size=0),strip.text=element_text(size=20),axis.title=element_text(size=20))
  ggsave(filename = "umap_celltype_2.pdf", plot = p1, device = 'pdf', width = 27, height = 25, units = 'cm')
...
###为分群重新指定细胞类型 
current.cell.ids <- c("Astrcytes","Endothelial","Ependymal Cells","Lymphocytes","Meninges","Microglia","Neurons", "Oligodendrocytes","OPC","Pericytes","Schwann")
new.cell.ids <- c("Astro","Endo","Epend","Lym","Meninges","","Aging","Adult","Adult") 
...
scRNA@meta.data[["celltype"]]<-factor(scRNA@meta.data[["celltype"]], levels=c("ExN","InN","MN","Astro","Micro","OPC","MOL","Schwann","Epend","Endo","Peri","Meninges","T"))
###为分群重新指定细胞类型 
cluster_celltype <-  data.frame(readxl::read_xlsx("cluster_celltype.xlsx"))
current.cluster.ids <- cluster_celltype$cluster
...
markers.to.plot <- c("RBFOX1","SNAP25","SLC17A7","CAMK2A","GAD1","GAD2","DCX","RELN","MPZ","PMP22",
                     "GFAP","AQP4","CSF1R","CD74","VCAN","SOX6","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","PDGFRB","ITGA10","PTPRC","CD247","CLIC6","DNAH9","CFAP54")
...
dev.off()
pdf("cluster_marker_3.pdf", width = 9,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_4.pdf", width = 9,height = 14)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

markers.to.plot <- c("RBFOX1","SNAP25","SYP","CAMK2A","GAD1","GAD2",
                     "GFAP","AQP4","CSF1R","CD74","VCAN","SOX6","PLP1","MBP","MPZ","PMP22","DNAH9","CFAP54","FLT1","CLDN5","PDGFRB","DCN","COL1A2","ITGA10","PTPRC","CD247")
pdf("celltype_marker_1.pdf", width = 9.5,height = 4.5)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "celltype", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("celltype_marker_2.pdf", width = 9.5,height = 4.5)
...
##保存数据
scRNA = scRNA[,scRNA$celltype %in% c("ExN","InN","Astro","Micro","OPC","NFOL","MOL","Endo","Peri")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA$celltype <- factor(scRNA$celltype,levels=c("ExN","InN","Astro","Micro","OPC","NFOL","MOL","Endo","Peri"))
...
save(scRNA,file="ACC_seurat.RData")
...
save(scRNA,file="/Users/niuruize/Downloads/scRNA/AD_aging/aging/SC/Seurat/SC_seurat.RData")
