...
#load data
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/Micro_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/AMY_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/EC_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/HIP_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/MB_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/PFC_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/Retina_Micro.RData")
load("~/Downloads/scRNA/AD_aging/aging/subtype/Microglia/SC_Micro.RData")

#合并数据
scRNA = merge(ACC_Micro, y=c(AMY_Micro,EC_Micro,HIP_Micro,MB_Micro,PFC_Micro,Retina_Micro,SC_Micro))
rm(ACC_Micro,AMY_Micro,EC_Micro,HIP_Micro,MB_Micro,PFC_Micro,Retina_Micro,SC_Micro)
save(scRNA,file="Micro_merge.RData")
...
save(scRNA,file="Micro.RData")

#第二次整合数据
scRNA = scRNA[,scRNA$seurat_clusters %in% c("0","1","3","6","8","9")]
...
prop.table(table(scRNA$seurat_clusters, scRNA$Group), margin = 2)*100
...
new.cluster.ids <- c("HM","TRM","ARM","TRM","HM","ARM","CPM") 
scRNA$celltype <- plyr::mapvalues(x = as.integer(as.character(scRNA$seurat_clusters)), from = current.cluster.ids, to = new.cluster.ids)
table(scRNA$celltype,scRNA$Group)
scRNA@meta.data[["celltype"]]<-factor(scRNA@meta.data[["celltype"]], levels=c("HM","TRM","ARM","CPM"))
prop.table(table(scRNA$celltype, scRNA$Group), margin = 2)*100
###为分群重新指定细胞类型 
current.cluster.ids <- c("0","1","2","3","4","5","6")
new.cluster.ids <- c("HM1","TRM1","ARM1","TRM2","HM2","ARM2","CPM") 
scRNA$celltype1 <- plyr::mapvalues(x = as.integer(as.character(scRNA$seurat_clusters)), from = current.cluster.ids, to = new.cluster.ids)
table(scRNA$celltype1,scRNA$Group)
prop.table(table(scRNA$celltype1, scRNA$Group), margin = 2)*100
scRNA@meta.data[["celltype1"]]<-factor(scRNA@meta.data[["celltype1"]], levels=c("HM1","HM2","TRM1","TRM2","ARM1","ARM2","CPM"))
...
ggsave(filename = "Heatmap10.pdf", plot = Heatmap, device = 'pdf', width = 30, height = 60, units = 'cm')
...
ggsave(filename = "Heatmap5_1.pdf", plot = Heatmap, device = 'pdf', width = 25, height = 30, units = 'cm')
Heatmap <- DoHeatmap(scRNA,features = top5$gene,disp.min=-2.5, disp.max=2.5, group.colors = c("#00BFC4","#AB82FF","#00CD00","#C77CFF"))+
  scale_fill_gradientn(colors = c("white","grey","firebrick3")) 
ggsave(filename = "Heatmap5_2.pdf", plot = Heatmap, device = 'pdf', width = 30, height = 30, units = 'cm')
...
markers.to.plot <- c("RBFOX1","SNAP25","SLC17A7","CAMK2A","GAD1","GAD2","DCX","RELN",
                     "GFAP","AQP4","CSF1R","CD74","VCAN","SOX6","OLIG1","OLIG2","BCAS1","PLP1","MBP","FLT1","CLDN5","DCN","COL1A2","PTPRC","CD247","CLIC6","DNAH9","CFAP54")
...
dev.off()
pdf("cluster_marker_3.pdf", width = 8,height = 4)
DotPlot(scRNA, features = markers.to.plot, cols = c("blue", "red","green"), group.by = "seurat_clusters", dot.scale = 8) +  RotatedAxis()
dev.off()
pdf("cluster_marker_4.pdf", width = 8,height = 4)
DotPlot(scRNA, features = markers.to.plot, cols = c("lightgrey", "red"),group.by = "seurat_clusters", col.min = 0,col.max = 2.5, dot.scale = 8) + RotatedAxis()
dev.off()

markers.to.plot <- c("PTPRC","TMEM119","P2RY12","SALL1","CX3CR1","C1QA","C1QB","C1QC","sct_APOE","CST7","CLEC7A","SPP1","LPL","HIF1A","IGF1","P2RY6","CD74","IFIT2","IFIT3","TOP2A","MCM2","MKI67")
...
##保存数据
scRNA = scRNA[,scRNA$celltype %in% c("ExN","InN","Astro","Micro","OPC","MOL","Endo","Peri")]
scRNA$celltype <- as.factor(as.character(scRNA$celltype))
scRNA$celltype <- factor(scRNA$celltype,levels=c("ExN","InN","Astro","Micro","OPC","MOL","Endo","Peri"))
...
save(scRNA,file="Micro_seurat.RData")
...
save(scRNA,file="scRNA_Micro_seurat.RData")
